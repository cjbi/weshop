...
# 网关服务jar包名称
export api_gateway_jar_name=weshop_api_gateway-@project.version@.jar
...
echo "注册中心服务旧应用进程id：$eureka_server_pid"
...
echo "配置中心服务旧应用进程id：$config_server_pid"
...
echo "网关服务旧应用进程id：$api_gateway_pid"
if [[ -n "$api_gateway_pid" ]]
then
  echo "网关服务服务旧进程：$api_gateway_pid"
